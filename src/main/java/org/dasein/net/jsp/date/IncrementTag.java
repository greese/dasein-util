...
public class IncrementTag extends TagSupport {
    ...
    public void setCalendar(String cal) throws JspException {
        Object ob = (new ELParser(cal)).getValue(pageContext);
    ...
    public void setIncrement(String inc) throws JspException {
        Number i = (new ELParser(inc)).getNumberValue(pageContext);
    ...
    public void setPeriod(String per) throws JspException {
        per = (new ELParser(per)).getStringValue(pageContext);
        per = per.toUpperCase();
        if( per.equals("DAY") ) {
            period = Calendar.DAY_OF_MONTH;
        }
