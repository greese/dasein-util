...
public class EnumTag extends TagSupport {
    ...
    @SuppressWarnings("unchecked")
    public void setEnumName(String nom) throws JspException, ClassNotFoundException {
        nom = (new ELParser(nom)).getStringValue(pageContext);
        if( nom != null ) {
            try {
    ...
    public void setValue(String val) throws JspException {
        all = false;
        value = (new ELParser(val)).getStringValue(pageContext);
    }
